            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        <h5><a href="{{route('admin.stores.create')}}"><i class="ik ik-plus bg-blue"></i> Add New</a>
                        </h5>
                        <div class="d-inline">
                            <h5>Stores</h5>
                            ...
                            <table id="simpletable" class="table table-striped table-bordered nowrap">
                                ...
                                <tbody>
                                    ...
                                    <tr id="tbl_record_{{$row->id}}">
                                        <td>{{$loop->index + 1 }}</td>
                                        <td>{{$row->title }}</td>
                                        <td>{{ $row->address_line_1 }}, {{ $row->address_line_1 }}, {{ $row->city }}, {{ $row->state }}, {{ $row->country }}, {{ $row->pincode }}</td>
                                        <td>{{$row->status }}</td>
                                        <td>
                                            @php $id = $row->id; @endphp
                                            <a href="{{ route('admin.stores.edit',[base64_encode($id)]) }}"
                                                title="Edit"><i class="ik ik-edit f-16 mr-15 text-green"></i></a>
                                            <a href="javascript:void(0)" class="ajax_delete_store"
                                                id="store_{{$row->id}}"><i class="ik ik-trash-2 f-16 text-red"
                                                    data-toggle="tooltip"
...
<script>
//delete blog ajax
$(document).on('click', '.ajax_delete_store', function(e) {
    ...
    if (idFor != '') {
        swal({
            ...
            function() {
                ...
                $.ajax({
                    url: "{{route('admin.stores.destroy',['" + idFor + "'])}}",
                    data: {
                        idFor: idFor
                    },
                    type: "DELETE",
...
//blog delete
$(document).on('click', '.store_change_status', function(e) {
    var mixbtnId = $(this).attr('id');
    var explode_btnid = mixbtnId.split('_');
    var statusFor = explode_btnid[0];
    var idFor = explode_btnid[1];
                    ...
                    success: function(res) {
                        ...
                        if (GetResponse == 1) {
                            ...
                            if (statusNew == 'inactive') {
                                ...
                                $("#span_status_" + idFor).html(
                                    '<a href="javascript:void(0)" class="store_change_status" id="' +
                                    statusFor + '_' + idFor +
