                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    if($product_rating == 1){
                                     ...
                                     <div class="rate">
    <input type="radio" id="star5_<?=$loop->index?>" class="rating_<?=$loop->index?>" name="rate" data-product="<?=$prod_id?>" value="5" checked/>
    <label for="star5_<?=$loop->index?>" title="5">5 stars</label>
    <input type="radio" id="star4_<?=$loop->index?>" class="rating_<?=$loop->index?>" name="rate" data-product="<?=$prod_id?>" value="4" />
    <label for="star4_<?=$loop->index?>" title="4">4 stars</label>
    <input type="radio" id="star3_<?=$loop->index?>" class="rating_<?=$loop->index?>" name="rate" data-product="<?=$prod_id?>" value="3" />
    <label for="star3_<?=$loop->index?>" title="3">3 stars</label>
    <input type="radio" id="star2_<?=$loop->index?>" class="rating_<?=$loop->index?>" name="rate" data-product="<?=$prod_id?>" value="2" />
    <label for="star2_<?=$loop->index?>" title="2">2 stars</label>
    <input type="radio" id="star1_<?=$loop->index?>" class="rating_<?=$loop->index?>" name="rate" data-product="<?=$prod_id?>" value="1" />
    <label for="star1_<?=$loop->index?>" title="1">1 star</label>
  </div>
  @push('custom-scripts')
  <script>
    $(document).ready(function(){
        $(".rating_<?=$loop->index?>").on('click',function(){
            var star = $(this).val();
