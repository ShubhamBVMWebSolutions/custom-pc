...
<?php $price_val = $_GET['price'] ?? '';
      ...
      if(!empty($price_val)){
          ...
          $min_price_new = $price_arr[0];
          $max_price_new = $price_arr[1];
      }
      if(isset($_GET['price'])){
...
$(document).ready(function(){
    ...
    $("#min_amount").keypress(function(){
		...
		const curr_url = new URL(location);
		var url = curr_url+'?price='+min_amount+'-'+max_amount;
    ...
    $( "#slider-range" ).slider({
	  ...
	  slide: function( event, ui ) {
		...
		const curr_url = new URL(location);
		curr_url.searchParams.delete('color');
		curr_url.searchParams.delete('price');
		curr_url.searchParams.delete('graphic');
		curr_url.searchParams.delete('processor');
		curr_url.searchParams.delete('ram');
		curr_url.searchParams.delete('screen_size');
		curr_url.searchParams.delete('ssd');
		curr_url.searchParams.delete('type');
		
		var url = curr_url+'?price='+ui.values[ 0 ]+'-'+ui.values[ 1 ];
		window.location.href = url;
	  }
	});
