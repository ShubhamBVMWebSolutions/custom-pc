...
class ProductCollectionController extends Controller
{
    ...
    public function index($slug){
        try {
            ...
            if($sortby == 'price_asc'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.price',"ASC");
            }
            if($sortby == 'price_desc'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.price',"DESC");
            }
            if($sortby == 'new_old'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.id',"DESC");
            }
            if($sortby == 'old_new'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.id',"ASC");
            }
            if($sortby == 'title_asc'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.title',"ASC");
            }
            if($sortby == 'title_desc'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.title',"DESC");
            }
            if($sortby == 'popular'){
                $query = Product::where("status",1)->where('product_type','product')->orderBy('products.views',"DESC");
            }
            
            // product collection
            
            $product_collection = ProductCollection::where("status","Active")->where("slug",$slug)->first();
            
            if(empty($product_collection)){
                return abort(404);
            }
            $metaDetails = $product_collection;
            if(!empty($product_collection->product_ids)){
                $productCollectionProIdArray = json_decode($product_collection->product_ids,true);
                $query->whereIn("products.id",$productCollectionProIdArray);
            } 
