...
class HomeController extends Controller
{
    ...
    { 
        ...
        $featured_products = Product::where(['featured'=>'yes','status'=>1,'product_type'=>'product'])->orderBy('id','DESC')->paginate(6);
        $homesection = HomeSection::get();
        $selling_products = OrderItems::select('product_id')->groupBy('product_id')->orderByRaw('COUNT(*) DESC')->get();
        // dd($selling_products);
        $data = compact('homeslider','homelogos','testimonials','gettingsdata','featured_products','selling_products','homesection');
