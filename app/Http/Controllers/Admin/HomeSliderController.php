    	      ...
    	      if(!empty($homesliderid))
    		     {
                    ...
                    $homeslider = HomeSlider::find($homesliderid);
       		  	   ...
       		  	   $background_image = $request->old_post_background_image;
                   ...
                   if ($validator->fails())
		        {
	             ...
	             if($request->hasFile('background_image'))
	             {
    	            $dir = public_path('/').Config::get('constants.HOME_SLIDER_IMAGE_PATHS');
    	             //->delete old image 
    	            if(file_exists(base_path($dir.$background_image)))
                    {
                         \File::delete(base_path($dir.$background_image));
                    }

    	            $background_image = time().'.'.$request->background_image->getClientOriginalExtension();
    	            $uploadImg = $request->file('background_image')->move($dir , $background_image);
    	            $homeslider->background_image       =     $background_image;
	             }
