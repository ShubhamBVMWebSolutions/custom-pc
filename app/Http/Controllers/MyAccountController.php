...
class MyAccountController extends Controller
{
    function dashboard(){
        ...
        $conversation_details = Conversation::where("second_person_id",$user_id)->first();
        if(!empty($conversation_details)){
            $message = Message::where("conversation_id",$conversation_details->id)->where("receiver_id",$user_id)->where("sender_type","Admin")->where("is_seen","No")->first();
            if(!empty($message)){
                $message_status = true;
            }
        }
        $data = compact('user_details','message_status');
        ...
        if($request->isMethod('POST')){
            ...
            if(isset($request->curr_password) || isset($request->new_password)){
                $rules = array(
                    ...
                    if(!Hash::check($request->curr_password,$userInfo->password)){
                        return redirect()->back()->with('error_message','Current password not matched.');
                    }
            ...
            if($isSaved){
              return redirect()->back()->with('success_message','Profile Updated Successfully!');
               ...
               return redirect()->back()->with('error_message','Profile Update Failed');
            }
        }
    ...
    function allorders(){
        ...
        if(isset($_GET['action']) && $_GET['action'] == 'cancel'){
          ...
          $update_status->save();
          session()->flash('success_message','Order Cancelled.');
    ...
    public function productReview(Request $request){
            ...
            $product_rating->user_id = auth()->user()->id;
            $product_rating->product_id = $product_id;
            $product_rating->review_title = $review_title;
            $product_rating->review_content = $review_content;
            $product_rating->recommend_product = $recommend_product;
            $product_rating->quality_rating = $quality_rating;
            $product_rating->value_for_money_rating = $value_for_money_rating;
            $product_rating->save();
            session()->flash("success_message","Review added.");
